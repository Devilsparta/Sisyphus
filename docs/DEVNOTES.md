...
pnpm --filter @sisylabs/daemon dev
...
- M25.1: `@sisylabs/kernel` ready for npm publish (scope `@sisyphus` is free; verified via `npm view`).
  ...
  - Next step (manual): `npm login` → `cd packages/kernel && pnpm publish` (or `pnpm -F @sisylabs/kernel publish` from root). Once on npm, `plugin-base` / `plugin-todo` can publish too and the marketplace install button will start working for the reference plugins.
...
- Full smoke chain now 57 assertions across 5 suites; `pnpm --filter @sisylabs/daemon test:smoke` builds plugin-hello, runs broker/migration/lifecycle/spawnagent under tsx dev, rebuilds the SEA binary, runs sea-smoke. All green.
...
2. **Publish `@sisylabs/kernel` to npm**: outside plugin authors need `import type { SisyphusPlugin }` — workspace dep blocks them.
